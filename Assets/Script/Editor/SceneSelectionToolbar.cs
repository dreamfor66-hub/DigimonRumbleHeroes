...
public static class SceneSelectionToolbar
{
    ...
    static void OnToolbarGUI()
    {
        ...
        if (GUILayout.Button("X"))
            ClearScene(_sceneOpened);

        var selectedIndex = EditorGUILayout.Popup(_selectedIndex, _displayedOptions); ;
        ...
        GUI.enabled = true;
        if (GUI.changed && selectedIndex > 0 && _scenes.Count > selectedIndex - 1 && !EditorApplication.isPlaying)
        {
    ...
    static void ClearScene(SceneInfo scene)
    {
        if (_scenes == null)
